...
function validarSolicitacao(funcionario, periodos, abonoPecuniarioDias) {
  ...
  // Política interna da empresa: no máximo 2 períodos (a CLT permitiria até 3, mas a empresa
  // optou por utilizar apenas 2).
  if (!Array.isArray(periodos) || periodos.length < 1 || periodos.length > 2) {
    return { valid: false, error: "As férias devem ser divididas em no mínimo 1 e no máximo 2 períodos." };
  }
  ...
  // Preferência de adiantamento da 1ª parcela do 13º salário nas férias (Lei 4.749/1965, art. 2º,
  // § único). Este sistema apenas registra a preferência do funcionário na programação — o
  // requerimento formal e o lançamento oficial ficam a cargo do RH. Se não solicitado, o
  // funcionário recebe o 13º normalmente, na data padrão.
